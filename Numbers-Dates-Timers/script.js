...
const formatCur = function(value, locale, currency){
  return new Intl.NumberFormat(locale, {
    style: 'currency',
    currency: currency, // taking the locale form the account and it formats it automatically
  }).format(value);
}
...
///////////////////////////////////////
// Event handlers
let currentAccount, timer; //making them global variables->so the timer persist during different logins // they need to be in the parent scope
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    const options = {
      ...
      minute: 'numeric',
      day: 'numeric',
      month: 'numeric', //we can use long also long/ 2-digit/numeric
      year: 'numeric',
      //weekday: 'long',
    }
    
    // getting the region from the user's browser
    //const locale = navigator.language; 
    //console.log(locale);

    labelDate.textContent = new Intl.DateTimeFormat //this is the option for formating the international dates, write it as a function in order give it parameter of which country and it will automatically adjust it
    (currentAccount.locale, options).format(now2);
    // using the account locate to create the date for it
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    //Reset Timer
    clearInterval(timer);
     //its important thats outside of the handler functions, we need it for other operations/callback functions
    timer = startLogoutTimer();
  }
});
...
//Internationalzing numbers 

const numz = 2883838383.23

const options2 = {
  style: "currency", //unit, percent || there are 3 options for style- currency, unit and percent
  unit: 'celsius', // mile-per-hour // if we have currency unit is completely ignored
  currency: 'EUR',
  //useGrouping: false,
}

console.log('US: ', new Intl.NumberFormat('en-US', options2).format(numz));
console.log('Germany:',new Intl.NumberFormat('de-DE', options2).format(numz));
